...
impl TcpHdr {
    ...
    pub fn ack_seq(&self) -> u32 {
        // SAFETY: Pointer arithmetic in bounds of the struct.
        unsafe { getter_be!(self, ack_seq, u32) }
    }
    ...
    pub fn set_ack_seq(&mut self, ack_seq: u32) {
        // SAFETY: Pointer arithmetic in bounds of the struct.
        unsafe { setter_be!(self, ack_seq, ack_seq) }
    }
    ...
    pub fn window(&self) -> u16 {
        // SAFETY: Pointer arithmetic in bounds of the struct.
        unsafe { getter_be!(self, window, u16) }
    }
    ...
    pub fn set_window(&mut self, window: u16) {
        // SAFETY: Pointer arithmetic in bounds of the struct.
        unsafe { setter_be!(self, window, window) }
    }
    ...
    pub fn urg_ptr(&self) -> u16 {
        // SAFETY: Pointer arithmetic in bounds of the struct.
        unsafe { getter_be!(self, urg_ptr, u16) }
    }
    ...
    pub fn set_urg_ptr(&mut self, urg_ptr: u16) {
        // SAFETY: Pointer arithmetic in bounds of the struct.
        unsafe { setter_be!(self, urg_ptr, urg_ptr) }
    }
    ...
    /// Returns all TCP flags as a single byte value.
    /// 
    /// This method returns the raw flags byte containing all TCP control flags
    /// (FIN, SYN, RST, PSH, ACK, URG, ECE, CWR). Useful for exact flag matching.
    /// 
    /// # Returns
    /// The complete flags byte from the TCP header.
    #[inline]
    pub fn get_flag_only(&self) -> u8 {
        self.off_res_flags[1]
    }

    /// Sets all TCP flags as a single byte value.
    /// 
    /// This method sets the raw flags byte containing all TCP control flags
    /// (FIN, SYN, RST, PSH, ACK, URG, ECE, CWR). Useful for exact flag setting.
    /// 
    /// # Parameters
    /// * `flags` - The complete flags byte to set in the TCP header
    #[inline]
    pub fn set_flag_only(&mut self, flags: u8) {
        self.off_res_flags[1] = flags;
    }

    /// Tests if a specific TCP flag is set.
    /// 
    /// This method checks whether the specified flag bit is set in the TCP header.
    /// It performs a bitwise AND operation to test for the presence of the flag.
    /// 
    /// # Parameters
    /// * `mask` - The flag mask to test (e.g., 0x02 for SYN, 0x10 for ACK)
    /// 
    /// # Returns
    ...
    /// Helper method to set or clear a flag bit in the second byte of `off_res_flags`.
    #[inline]
    ...
    /// Returns the reserved bits from the TCP header.
    /// 
    /// This method extracts the 4 reserved bits from the off_res_flags field.
    /// These bits are typically zero but may be used by TCP extensions.
    /// 
    /// # Returns
    /// The 4 reserved bits as a u8 value (0-15).
    #[inline]
    pub fn res(&self) -> u8 {
        let off_res_flags = unsafe { getter_be!(self, off_res_flags, u16) };
        ((off_res_flags >> 8) & 0x0F) as u8
    }

    /// Sets the reserved bits in the TCP header.
    /// 
    /// This method sets the 4 reserved bits in the off_res_flags field.
    /// These bits are typically zero but may be used by TCP extensions like AccECN.
    /// 
    /// # Parameters
    /// * `res` - The reserved bits value to set (0-15, only lower 4 bits are used)
    #[inline]
    pub fn set_res(&mut self, res: u8) {
    self.off_res_flags[0] = (self.off_res_flags[0] & 0xF0) | (res & 0x0F);
    }
